...
uint8_t ppu_readb(region_data_t *data, uint16_t address)
{
	...
	case STATUS_REG_ADDR:
		/* SR reads reset ports 2005/2006 first write flipflop */
		ppu->flipflop_first_write = true;

		/* Reading status register during VBLANK clears the flag but we
		should still return its old state */
		b = ppu->regs[STATUS_REG_ADDR];
		if (ppu->status_reg.vblank_flag)
			ppu->status_reg.vblank_flag = 0;
		return b;
	case SPR_RAM_DATA_REG_ADDR:
		/* Read from sprite RAM */
		return ppu->spr_ram[ppu->regs[SPR_RAM_ADDR_REG_ADDR]];
	case VRAM_READ_WRITE_DATA_REG_ADDR:
		/* During first frame after reset, port is read-protected */
		if (ppu->first_frame)
			return 0;
		...
		ppu->vram_address +=
			...
			32 : 1;
		return b;
	case CONTROL_REG_1_ADDR:
	case CONTROL_REG_2_ADDR:
	case SPR_RAM_ADDR_REG_ADDR:
	case BG_SCROLLING_OFFSET_ADDR:
	case VRAM_ADDR_REG_ADDR:
	default:
...
void ppu_writeb(region_data_t *data, uint8_t b, uint16_t address)
{
	...
	switch (address) {
	case CONTROL_REG_1_ADDR:
		/* During first frame after reset, port is write-protected */
		if (!ppu->first_frame)
			ppu->regs[CONTROL_REG_1_ADDR] = b;
		return;
	case CONTROL_REG_2_ADDR:
		/* During first frame after reset, port is write-protected */
		if (!ppu->first_frame)
			ppu->regs[CONTROL_REG_2_ADDR] = b;
		return;
	case SPR_RAM_ADDR_REG_ADDR:
		/* Specifies sprite RAM address for later access */
		ppu->regs[SPR_RAM_ADDR_REG_ADDR] = b;
		return;
	case SPR_RAM_DATA_REG_ADDR:
		/* Store byte to sprite RAM */
		ppu->spr_ram[ppu->regs[SPR_RAM_ADDR_REG_ADDR]] = b;

		/* Sprite RAM address is incremented after every data write */
		ppu->regs[SPR_RAM_ADDR_REG_ADDR]++;
		return;
	case BG_SCROLLING_OFFSET_ADDR:
		/* During first frame after reset, port is write-protected */
		if (!ppu->first_frame)
			ppu->regs[BG_SCROLLING_OFFSET_ADDR] = b;
		return;
	case VRAM_ADDR_REG_ADDR:
		/* During first frame after reset, port is write-protected */
		if (ppu->first_frame)
